...
class ReviewStatsResponse(BaseModel):
    ...
    total_reviews: int = Field(ge=0)
    average_rating: float = Field(ge=0, le=5)
    response_rate: float = Field(ge=0, le=100, description="Percentage of reviews with responses")
...
class TopPost(BaseModel):
    ...
    engagement_score: int = Field(ge=0, description="Computed score: likes + comments*2 + reach/10")
...
class PendingReview(BaseModel):
    ...
    ai_response: str | None = Field(default=None, description="Previously generated AI response if any")
...
class PublishResponseRequest(BaseModel):
    ...
    final_response: str = Field(min_length=1, max_length=5000, description="The approved response text to publish")
